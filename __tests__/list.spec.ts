...
describe("List", () => {
  it("should handle simple struct", () => {
    expect(compileExample(FOO, "./src/examples/list.ts")).toStrictEqual(["a: u8", "b: bool", "i: i32"]);
  });
  it("should list fields defined in constructor", () => {
    expect(compileExample(VEC, "./src/examples/list.ts")).toStrictEqual(["x: f64", "y: i64", "z: u32"]);
  });
});